...
1. Historical ControlSim and GKE both used `gamma=0.90`; this remains provenance only, not proof that `0.90` is appropriate for Q-AHBN2. AR-1 retains future-value bootstrapping ($\gamma>0$) but reopens the exact numerical value. The bounded candidate set is $\Gamma=\{0.70,0.80,0.90\}$, with `0.90` retained as the historical-baseline candidate. Final selection is deferred until the next-same-peer transition implementation passes readiness tests and the predeclared minimal sensitivity protocol is executed.
2. `alpha_Q=0.25`, `epsilon_min=0.03`, and `epsilon_decay=0.995` are common historical values across both environments; their historical agreement must not be represented as RO2 proof of numerical optimality.
